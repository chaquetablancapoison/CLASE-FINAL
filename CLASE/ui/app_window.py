...
class AppWindow(tk.Tk):
    def __init__(self, service: UserService) -> None:
        ...
        self.title("Registro de Usuarios - .grid()")
    ...
    def create_widgets(self):
        ...
        ttk.Label(
            ...
            font=("Arial", 18, "bold")
        ).grid(row=0, column=0, columnspan=2, padx=10, pady=(20, 15))

        ttk.Label(self, text="Nombre:").grid(
            row=1, column=0, padx=10, pady=7, sticky="e")
        self.entry_fname = ttk.Entry(self, width=40)
        self.entry_fname.grid(
            row=1, column=1, padx=10, pady=7, sticky="w")

        ttk.Label(self, text="Apellido:").grid(
            row=2, column=0, padx=10, pady=7, sticky="e")
        self.entry_lname = ttk.Entry(self, width=40)
        self.entry_lname.grid(
            row=2, column=1, padx=10, pady=7, sticky="w")

        ttk.Label(self, text="Edad:").grid(
            row=3, column=0, padx=10, pady=7, sticky="e")
        self.entry_age = ttk.Entry(self, width=40)
        self.entry_age.grid(
            row=3, column=1, padx=10, pady=7, sticky="w")

        ttk.Label(self, text="Correo:").grid(
            row=4, column=0, padx=10, pady=7, sticky="e")
        self.entry_email = ttk.Entry(self, width=40)
        self.entry_email.grid(
            row=4, column=1, padx=10, pady=7, sticky="w")

        ttk.Button(
            self, text="Ingresar usuario",
            command=self.create_new_user
        ).grid(row=5, column=0, columnspan=2, padx=10, pady=12)
    ...
    def render_data_table(self):
        ...
        for user in self.service.find_all():
            self.tree.insert(
                "", "end",
                values=(user.fname, user.lname, user.age, user.email)
            )


if __name__ == "__main__":
    # La aplicación se inicia desde main.py.
    pass
